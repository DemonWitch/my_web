...
public class ShiroUtil {
    ...
    public static User getSubject() {
        ...
        // 初始化延迟加载的部门信息
        if (user != null && !Hibernate.isInitialized(user.getCreateDate())) {
            try {
                Hibernate.initialize(user.getCreateDate());
            } catch (LazyInitializationException e) {
                // 部门数据延迟加载超时，重新查询用户数据（用于更新“记住我”状态登录的数据）
                UserService userService = SpringContextUtil.getBean(UserService.class);
                User reload = userService.getById(user.getId());
                Hibernate.initialize(reload.getCreateDate());
                // 将重载用户数据拷贝到登录用户中
                BeanUtils.copyProperties(reload, user);
            }
        }
